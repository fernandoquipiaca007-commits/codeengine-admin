...
/**
 * Singleton for database/storage operations (anon key).
 * SECURITY: We must NEVER use the service role key in the browser bundle
 * as it bypasses Row Level Security (RLS). All frontend data operations
 * must use the anon key and rely on server-side RLS for protection.
 */
