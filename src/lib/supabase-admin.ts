 ...
 * Always returns the authenticated 'anon' client to enforce Row Level Security (RLS).
 * Security: Service role key was removed from frontend to prevent administrative bypass.
 */
...
export function handleSupabaseError(error: unknown, context: string): never {
  // Security: Log original error to console for developers, but throw a generic message
  // to avoid leaking database schema details to the UI/end-user.
