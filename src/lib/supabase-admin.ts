...
/**
 * 🛡️ SECURITY NOTE:
 * We NEVER use VITE_SUPABASE_SERVICE_ROLE_KEY in the frontend.
 * The service role key bypasses Row Level Security (RLS) and must remain server-side.
 * All admin operations here must use the authenticated 'anon' client,
 * which is governed by RLS policies based on the user's JWT.
 */
 ...
 * 🛡️ Returns the authenticated 'anon' client to enforce Row Level Security (RLS).
 */
