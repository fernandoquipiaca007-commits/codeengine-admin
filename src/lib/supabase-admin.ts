 ...
 * Also used for data operations to ensure Row Level Security (RLS) is enforced.
 */
 ...
 * Sentinel: Removed service role key usage to prevent administrative bypass in the browser.
 */
...
/**
 * Sanitizes Supabase errors before throwing them to the UI.
 * Sentinel: Prevents technical information leakage.
 */
export function handleSupabaseError(error: unknown, context: string): never {
  console.error(`[supabase] ${context}:`, error);
  // Throw a generic message to avoid leaking database internals/schema
