 ...
 * Now always uses the authenticated anon client to enforce RLS.
 * Never use .auth on this client.
 */
export function getDataClient(): SupabaseClient {
  if (!dataClientInstance) {
    // We always use getAuthClient() which uses the anon key.
    // RLS policies must be configured on Supabase to allow access based on the user session.
